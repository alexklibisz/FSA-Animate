...
ForceFSM.prototype = {
    ...
    restart: function() {
        ...
        //layout hasn't been initialized yet
    ...
    addTransition: function(name, source, target) {
        ...
        var sourceObj = this.states.find(source),
            ...
            check = this.transitions.find(transition.id);
        
        // if this transition already exists, combine
        // the labels and update it.
        if(check !== false) {
        	check.merge(transition);
        	transition = check; 
        }
...
function FSMTransition(name, source, target) {
    name = name.replace(/\s/g, '').split(',').sort().join(',');
    this.id = [source.name, target.name].join("_");
...
FSMTransition.prototype.merge = function(newTransition) {
	console.log("merge", this.name, newTransition.name);
	this.name += ',' + newTransition.name;
	this.name = this.name.replace(/\s/g, '').split(',').sort().join(',');
}
