function ForceGraph(element, width, height) {
    ...
    this.update = function() {
        ...
        if (this.forceOptimize) {
            force.distance(((width + height) / 2) / nodes.length);
        }
    ...
    this.addLink = function(id, sourceId, targetId) {
        var sourceNode = findNode('id', sourceId),
            targetNode = findNode('id', targetId);
        ...
        if (existingLink !== null && existingLink.id.indexOf(id) === -1) {
            this.removeLink(existingLink.source, null);
            this.removeLink(null, existingLink.target);
            existingLink.id = generateId(existingLink.id + "," + id);
    ...
    var generateId = function(id) {
        return id.replace(/\s/g, '').split(',').sort().join(',');
    }
