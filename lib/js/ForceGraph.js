function ForceGraph(element, width, height) {
    ...
    this.nodeRadius = ((width + height) / 2) * .03,
    ...
    this.addNode = function(id) {
        ...
        console.log(JSON.stringify(new node(id, this.nodeRadius)));
    ...
    this.addLink = function(id, sourceId, targetId) {
        ...
        var existingLink = findLink(sourceId, targetId);
        if (existingLink !== null && existingLink.id.indexOf(id) === -1) {
            this.removeLink(existingLink.source, existingLink.target, 2);
            existingLink.id = [existingLink.id.split(','), id].sort().join(',');
            this.addLink(existingLink.id, existingLink.source.id, existingLink.target.id);
        } else {
            links.push({
                "id": id.split(',').sort().join(','),
                "elementId": ["L", sourceNode.id, targetNode.id].join("-"),
                "source": sourceNode,
                "target": targetNode
            });
        }
     ...
     * 
     * truth is a variable 1 or 2.
     * if truth is 1, only the source or the target must match to delete the link.
     * if truth is 2, the source and the target must match to delete the link.
     *
     */
    this.removeLink = function(source, target, truth) {
        var i = 0;

        if(truth === undefined) truth = 1;
            
        // also accept the source and target id
        if (typeof(source) !== 'object') source = findNode(source);
        if (typeof(target) !== 'object') target = findNode(target);
...
function node(id, nodeRadius) {
    this.id = id.replace(/\s/g, '').split(',').sort().join(',');
    this.elementId = 'N' + id;
    this.selected = false;
