...
Converter.prototype.stepForward = function() {
  var states, sigma, delta, initState, finalStates;
  var tmp_array = [];
  ...
  if (this.curstep === 0) {
    ...
    finalStates = this.dfa.finalStates;
    ...
    console.log("eclosed_transitions = ", tmp_array);
    if (tmp_array.length === 0) {
      delta.put(states[this.i]+'-'+sigma[this.j], '?');
      console.log("adding transition: "+states[this.i]+'-'+sigma[this.j]+" --> ?");
    } else {
...
Converter.prototype.reset = function() {
  if(this.nfa != undefined) this.nfa.reset();
